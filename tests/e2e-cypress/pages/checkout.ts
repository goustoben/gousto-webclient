...
class Checkout extends Page {
  ...
  createGoustoCompliantPassword() {
    return `${random.alphaNumeric(5)}${random.alpha({ count: 3 }).toUpperCase()}${datatype.number({
      max: 999,
    })}`
  }

  assertCheckoutConfirmation() {
    cy.findByText(/Congrats on ordering your first box/i).should('exist')
  }
  ...
  selectPaymentMethod(method: 'Card' | 'PayPal') {
    cy.get('input[name="paymentMethod"]').invoke('attr', 'value').should('equal', method).check()
  }
  ...
  selectDeliveryDetailsInstruction() {
    cy.selectNthOption('select[data-testing="checkoutDeliveryDetailsInstruction"]', 1, {
      force: true,
    })
  }
  ...
  enterPhoneNumber() {
    cy.get('[data-testing="checkoutPhoneNumberInput"]').type(phone.phoneNumber('07777######'))
  }

  enterCardNumber(cardNumber = this.defaultData.payment.card.cardNumber) {
    cy.getIframeBody('iframe#cardNumber').find('input#checkout-frames-card-number').type(cardNumber)
  }

  enterNameOnCard() {
    cy.get('[data-testing="checkoutCardNameInput"]').type(`${name.firstName()} ${name.lastName()}`)
  }
