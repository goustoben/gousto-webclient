...
export default {
	orderUpdate: (orderId, recipes, coreDayId, coreSlotId, numPortions, orderAction) => (
	...
	orderUpdateDayAndSlot: (orderId, coreDayId, coreSlotId, slotId) => (
		async (dispatch, getState) => {
	...
	orderAssignToUser: (orderAction, existingOrderId) => (
		async (dispatch, getState) => {
			dispatch(statusActions.error(actionTypes.ORDER_SAVE, null))
			dispatch(statusActions.pending(actionTypes.ORDER_SAVE, true))
			const accessToken = getState().auth.get('accessToken')
	...
	orderCheckPossibleDuplicate: (orderId) => (
		async (dispatch, getState) => {
			try {
	...
	orderAddressChange: (orderId, addressId) => (
		async (dispatch, getState) => {
	...
	orderGetDeliveryDays: (cutoffDatetimeFrom, cutoffDatetimeUntil, addressId, orderId) => (
		async (dispatch, getState) => {
	...
	cancelOrderModalToggleVisibility: (visibility, orderId) => (
		(dispatch) => {
			dispatch({
	...
	cancelledAllBoxesModalToggleVisibility: (visibility) => ({
		type: actionTypes.CANCELLED_ALL_BOXES_MODAL_VISIBILITY_CHANGE,
		visibility,
	}),
}
