...
if [ "$npm_config_gousto_frontend_environmentName" == "production" ]
  then
    project=frontend-prod
fi

if [ "$npm_config_gousto_frontend_environmentName" == "staging" ]
  then
...
if [ "$npm_config_gousto_frontend_environmentName" = "production" ] || [ "$npm_config_gousto_frontend_environmentName" = "staging" ]
  then
    ...
    curl https://sentry.io/api/0/projects/$org/$project/releases/$releaseHash/files/ \
      ...
      -H "Authorization: Bearer $token" \
      -F file=@$mapFile \
      -F name="$npm_config_gousto_frontend_clientProtocol://$npm_config_gousto_frontend_cloudfrontUrl/$mapFile"
    ...
    curl https://sentry.io/api/0/projects/$org/$project/releases/$releaseHash/files/ \
      ...
      -H "Authorization: Bearer $token" \
      -F file=@$originalFile \
      -F name="$npm_config_gousto_frontend_clientProtocol://$npm_config_gousto_frontend_cloudfrontUrl/$originalFile"
    echo
    echo "[sentry] Uploaded file '$originalFile' to release '$releaseHash' as '$npm_config_gousto_frontend_clientProtocol://$npm_config_gousto_frontend_cloudfrontUrl/$originalFile'"
    echo
  else
    echo "[sentry] Skipping upload of sourcemaps as environment '$npm_config_gousto_frontend_environmentName' is not production"
fi
