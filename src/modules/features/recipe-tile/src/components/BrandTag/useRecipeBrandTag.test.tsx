...
describe("useRecipeBrandTag", () => {
  const images = [
    {
      type: "homepage-image",
      urls: [{ src: "a.png", width: 100 }],
      title: "",
      description: "",
    },
    {
      type: "mood-image",
      urls: [{ src: "b.png", width: 100 }],
      title: "",
      description: "",
    },
  ];
  ...
  const renderUseRecipeTaglineHook = ({ tagline }: { tagline: string }) => {
    ...
    const customWrapper: React.FC<customWrapperProps> = ({ children }) => (
      <RecipeContextProvider value={recipeWithTagline}>
        {children}
...
describe("useRecipeBrandTagline", () => {
  ...
  describe("when recipe is not found in context", () => {
    ...
    it("should return null", () => {
      const { result } = renderHook(() => useRecipeBrandTagline(), {
  ...
  describe("when recipe is found and has a tagline", () => {
    ...
    const localWrapper: React.FC<customWrapperProps> = ({ children }) => (
      <RecipeContextProvider value={localRecipe}>
        {children}
      </RecipeContextProvider>
    );
