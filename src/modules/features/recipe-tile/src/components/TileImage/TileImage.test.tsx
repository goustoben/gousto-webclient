...
const defaultGetAlternativeOptionsForRecipe = jest.fn().mockImplementation(() => [
  {
    recipeId: '111',
...
const renderComponent = ({
  ...
  render(
    <RecipeTileDependencies
      ...
      useGetAlternativeOptionsForRecipe={() => defaultGetAlternativeOptionsForRecipe}
      useStock={() => ({
...
describe('<TileImage />', () => {
  ...
  test('should contain one SoldOutOverlay component', () => {
    ...
    expect(screen.getByText(/This recipe is sold out for your delivery date/)).toBeInTheDocument()
  })

  test('should contain one CookingTimeIcon component', () => {
    const { container } = renderComponent({ cookingTime: 47 })
    expect(screen.getByText(/47/)).toBeInTheDocument()
    expect(container).toContainHTML('stroke-dasharray="78.33333333333333, 100"')
  })
