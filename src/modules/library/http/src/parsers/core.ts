...
export function parseValueToCoreData(value: unknown): CoreData<unknown> {
  if (!isRealObject(value)) {
    throw new ParseCoreDataError(`must be non-null object, was ${typeof value}=${value}`)
  }
  ...
  if (!isRealObject(value.result)) {
    throw new ParseCoreDataError(
      `*.result must be non-null object, was ${typeof value.result}=${value.result}`,
    )
  }
...
class ParseCoreDataError extends Error {
  constructor(reason: string) {
    super(`@library/http ParseCoreDataError: ${reason}`)
  }
}
