...
describe('collectionTransformer', () => {
  const menuServiceData: MenuServiceData = {
    isPending: false,
    error: null,
    data: {
      meta: {
  ...
  const menuData = menuServiceData.data!
  ...
  test('should transform data into format expected by UI', () => {
    ...
    const result = collectionsTransformer(menuData, menuData.data[0])
  ...
  describe('when menu does not have relationships', () => {
    test('should return empty collections', () => {
      ...
      const result = collectionsTransformer(menuData, newMenu)
