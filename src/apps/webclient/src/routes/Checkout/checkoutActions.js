...
import { fetchReference, customerSignup } from 'apis/customers'
...
import { PaymentMethod, signupConfig } from 'routes/Signup/signupConfig'
...
export { checkoutTransactionalOrder } from '../Menu/actions/checkout'

const { pending, error } = statusActions
const { errorsThatClearOrderPreview, errorsToHandleForSignupPayment, passwordRules } =
  checkoutConfig
...
export const fireCheckoutPendingEvent =
  (pendingName, checkoutValue = true) =>
  (dispatch) => {
    dispatch(pending(pendingName, checkoutValue))
  }

export const fireCheckoutError =
...
export function checkoutSignup({ pricing }) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      await dispatch(checkoutActions.userSubscribe({ pricing }))
    } catch (err) {
    ...
    if (isCardPayment(getState())) {
      dispatch(
        feLoggingLogEvent(
          logLevels.info,
          'checkoutSignup: userSubscribe finished, proceed to checkoutCardAuthorisation',
        ),
      )
      ...
      dispatch(
        feLoggingLogEvent(
          logLevels.info,
          'checkoutSignup: userSubscribe finished, proceed to checkoutSignupPayment',
        ),
      )
      const sourceId = null
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch(
        feLoggingLogEvent(
          logLevels.info,
          'checkoutSignupPayment: signupPayment succeeded, proceed to checkoutPostSignup',
        ),
      )
      ...
      dispatch(
        feLoggingLogEvent(
          logLevels.error,
          `checkoutSignupPayment: Signup payment failed: ${err.message}`,
        ),
      )
...
export const checkPaymentAuth =
  (checkoutSessionId, { pricing }) =>
...
export function checkoutPostSignup({ pricing }) {
  return async (dispatch, getState) => {
    ...
    if (loginError) {
      dispatch(
        feLoggingLogEvent(logLevels.info, `checkoutPostSignup: signup login failed: ${loginError}`),
      )
      dispatch(
        trackCheckoutError(
...
export function fetchPayPalClientToken() {
  return async (dispatch) => {
    try {
      ...
      logger.error({
        message: `${actionTypes.PAYPAL_TOKEN_FETCH_FAILED} - ${err.message}`,
        errors: [err],
      })
      dispatch(
...
export function setCurrentPaymentMethod(paymentMethod, options = {}) {
  return (dispatch, getState) => {
    ...
    if (
      ...
      dispatch(
        feLoggingLogEvent(logLevels.info, 'PayPal signup attempt failed because of init issue'),
      )
    }
