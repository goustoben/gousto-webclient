...
export const activeMenuForDate = (menuServiceData, date) => {
  ...
  return menuServiceData.data.find((menu) => moment(date) <= moment(menu.attributes.ends_at))
}
...
export const getMenuLimitsForBasket = createSelector(
  [getMenuLimits, getNumPortions, getBasketMenuId],
...
export const validateMenuLimitsForBasket = createSelector(
  [(_, recipeId) => recipeId || null, getBasketRecipes, getMenuLimitsForBasket],
