...
const listEnvironmentsForEndpoints = (endpoints: OldEndpointConfig.Endpoints) => Object.keys(endpoints)

const listServicesForEnvironments = (endpoints: OldEndpointConfig.Endpoints) => (environmentList: string[]) =>
  environmentList.map(env => Object.keys(endpoints[env].services).map(svc => [env, svc])).flat()

const listVersionsForServices = (cfg: OldEndpointConfig.Endpoints) =>
  (environmentServicesList: [string, string][]) =>
    environmentServicesList.map(([env, serviceName]) =>
...
const forLocalAndLive = (environmentServicesVersionList: [string, string, number][]) =>
  environmentServicesVersionList.map((arr) =>
    ['live', 'local'].map(liveOrLocal => [...arr, liveOrLocal])
  ).flat()

const forIsServerTrueAndFalse = (environmentServicesVersionListApi: [string, string, number, string][]) =>
  environmentServicesVersionListApi.map((arr) =>
    [true, false].map(b => [...arr, b])
  ).flat()
 ...
 */
function oldEndpoint(service: string, version = 1,
  {
    isCallFromServer, /* __SERVER__ */
    apiEnvironmentToPointTo, /* __API_ENV__ */
    appInstanceEnvironment /* __RUNNING_ENV__ */
  }: { isCallFromServer: boolean, apiEnvironmentToPointTo: string, appInstanceEnvironment: 'live' | 'local' } = {
...
const getExpectationFromEndpoint = (testCases: [string, string, number, 'live' | 'local', boolean][]) => testCases.map((ps: [string, string, number, 'live' | 'local', boolean]) => {
  const [apiEnvironmentToPointTo, serviceName, version, appInstanceEnvironment, isCallFromServer] = ps

  return [...ps, oldEndpoint(serviceName, version, {
    isCallFromServer,
    apiEnvironmentToPointTo,
...
export const generateEndpointTestCases = () => pipe(
