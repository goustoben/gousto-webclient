...
describe('SnakeToCamelCase generic type', () => {
  ...
  test('Literal "foo_bar_bam" -> "fooBarBam"', () => {
    ...
    type Output = SnakeToCamelCase<Input>
    const _typeCheck: TypesEqual<Output, 'fooBarBam'> = true
  })
  ...
  test('Literal "_prefixed" -> "Prefixed', () => {
    ...
    type Output = SnakeToCamelCase<Input>
    const _typeCheck: TypesEqual<Output, 'Prefixed'> = true
  })

  test('Literal "suffixed_" -> "suffixed_" (no change)', () => {
    type Input = 'suffixed_'
    type Output = SnakeToCamelCase<Input>
    const _typeCheck: TypesEqual<Output, 'suffixed_'> = true
  })
})
...
describe('SnakeCasedValue', () => {
  describe('given a record with string keys', () => {
    ...
    test('swaps snake_case keys', () => {
      const _typeCheck1: TypesEqual<Output, { snakeCase: true }> = true
      const _typeCheck2: TypesEqual<Output, { snakeCaseLong: true }> = true
    })

    test('transforms _prefixed keys', () => {
      const _typeCheck: TypesEqual<Output, { Prefixed: true }> = true
    })

    test('preserves other keys', () => {
      const _typeCheck: TypesEqual<Output, { camelCase: true; uncased: true }> = true
    })
  })
  ...
  describe('given an array with records', () => {
    ...
    test('transforms array items', () => {
      const _typeCheck: TypesEqual<
        Output,
        Array<Record<'snakeCase' | 'snakeCaseLong', string>>
      > = true
    })
  })
})
