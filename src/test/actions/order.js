...
describe('order actions', function() {
  ...
  describe('orderUpdate', function() {
    ...
    it('should mark ORDER_SAVE as errored if it errors', async function() {
      const err = new Error('oops')
      saveOrderSpy = () => new Promise((resolve, reject) => { reject(err) })
    ...
    it('should map the arguments through to saveOrder correctly', async function() {
      await orderActions.orderUpdate(orderId, recipes, coreDayId, coreSlotId, numPortions, orderAction)(dispatchSpy, getStateSpy)

      expect(saveOrderSpy.calledOnce).to.be.true
      const order = {
        recipe_choices: [
      ...
      expect(saveOrderSpy.getCall(0).args[0]).to.equal('access-token')
      expect(saveOrderSpy.getCall(0).args[1]).to.equal('12345')
      expect(saveOrderSpy.getCall(0).args[2]).to.deep.equal(order)
    })

    it('should redirect the user to the order summary page if it succeeds', async function() {
      orderAction = 'something'
      saveOrderSpy = () => new Promise(resolve => { resolve({ data: { id: '5678' } }) })
      orderActions = require('inject-loader?apis/orders&utils/window&./status!actions/order')({ // eslint-disable-line global-require
        'apis/orders': {
          saveOrder: saveOrderSpy,
        },
      ...
      expect(redirectSpy.withArgs('/order/5678/summary?order_action=something').calledOnce).to.be.true
    })

  })

  describe('orderUpdateDayAndSlot', function() {
    let orderId
    let coreDayId
    let coreSlotId
    let slotId
    ...
    it('should mark ORDER_UPDATE_DELIVERY_DAY_AND_SLOT as pending', async function() {
      await orderActions.orderUpdateDayAndSlot(orderId, coreSlotId, slotId)(dispatchSpy, getStateSpy)

      expect(pendingSpy).to.have.been.calledTwice
      expect(pendingSpy.getCall(0).args[0]).to.equal('ORDER_UPDATE_DELIVERY_DAY_AND_SLOT')
      expect(pendingSpy.getCall(0).args[1]).to.be.true
      expect(pendingSpy.getCall(1).args[0]).to.equal('ORDER_UPDATE_DELIVERY_DAY_AND_SLOT')
      expect(pendingSpy.getCall(1).args[1]).to.be.false
      expect(dispatchSpy.callCount).to.equal(4)
    })

    it('should mark ORDER_UPDATE_DELIVERY_DAY_AND_SLOT as errored if it errors', async function() {
      const err = new Error('oops')
      saveOrderSpy = () => new Promise((resolve, reject) => { reject(err) })
      ...
      await orderActions.orderUpdateDayAndSlot(orderId, coreDayId, coreSlotId, slotId)(dispatchSpy, getStateSpy)
      ...
      expect(errorSpy.calledTwice).to.be.true
      expect(errorSpy.getCall(0).args[0]).to.equal('ORDER_UPDATE_DELIVERY_DAY_AND_SLOT')
      expect(errorSpy.getCall(0).args[1]).to.be.null
      expect(errorSpy.getCall(1).args[0]).to.equal('ORDER_UPDATE_DELIVERY_DAY_AND_SLOT')
      expect(errorSpy.getCall(1).args[1]).to.deep.equal(err.message)
      expect(dispatchSpy.callCount).to.equal(4)
    })

    it('should map the arguments through to saveOrder and dispatch the action with the correct arguments', async function() {
      const updatedOrder = {
        data: {
      ...
      saveOrderSpy = sandbox.stub().returns(new Promise(resolve => { resolve(updatedOrder) }))
      orderActions = require('inject-loader?apis/orders&utils/window&./status!actions/order')({ // eslint-disable-line global-require
        'apis/orders': {
          saveOrder: saveOrderSpy,
        },
      ...
      expect(saveOrderSpy).to.have.been.calledOnce
      const order = {
        delivery_day_id: 3,
        delivery_slot_id: 8,
      }
