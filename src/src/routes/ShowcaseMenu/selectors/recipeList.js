...
import { getSelectedRecipeVariants, getCurrentMenuVariants } from 'routes/Menu/selectors/variants'
...
export const getRecipeComparatorFactory = createSelector(
  ...
  (inStockRecipes) => (recipes) => (a, b) => {
    ...
    const aIndex = recipes.indexOf(a)
    const bIndex = recipes.indexOf(b)
...
export const getFilterFn = createSelector(
  ...
  (collectionId, recommendations, inStockRecipes) => {
    const isRecommendationCollection =
      recommendations && collectionId === getCollectionId(recommendations)
...
const replaceRecipes = (recipeViews, replacements) => {
...
const createRecipesResponse = (recipeViews, filterFn, recipeComparatorFactory, replacements) => {
  ...
  const filteredViews = filterFn
    ? recipeViews.filter(({ recipe }) => filterFn(recipe))
    : recipeViews

  const compare = recipeComparatorFactory(filteredViews.map(({ recipe }) => recipe))
  const sortedViews = filteredViews.sort(({ recipe: recipeA }, { recipe: recipeB }) =>
    compare(recipeA, recipeB)
  )

  if (replacements) {
    const { recipes: replacedRecipes } = replaceRecipes(sortedViews, replacements)
...
const getSelectedVariantId = (selectedVariants, collectionId, recipeId) => {
  if (
    !selectedVariants ||
    !selectedVariants[collectionId] ||
    !selectedVariants[collectionId][recipeId]
  ) {
    return null
  }
...
const replaceOutOfStockWithVariants = (
  recipes,
  currentMenuRecipes,
  inStockRecipes,
  selectedVariants,
  ...
  recipes.map((recipe) => {
    ...
    const recipeVariants = getVariantsForRecipeForCurrentCollection(
      currentMenuVariants,
      recipeId,
      currentMenuRecipes,
      collectionDietaryClaims
    ...
    const inStockVariant = getInStockVariantRecipe(
      recipeVariants.alternatives,
      currentMenuRecipes,
      inStockRecipes
    )
...
export const getRecipeListRecipesLogic = (
  ...
  collectionId,
  filterFn,
  recipeComparatorFactory
) => {
  if (!collectionId) {
    // return all recipes on menu
    return createRecipesResponse(
      currentMenuRecipes.map(createStandardRecipeView),
      filterFn,
      recipeComparatorFactory
    )
  }
  ...
  const replacements = getReplacements(currentMenuRecipes, recipeIdsInCollection, (recipeId) =>
    getSelectedVariantId(selectedVariants, collectionId, recipeId)
  )

  return createRecipesResponse(
    recipesWithOutOfStockReplaced,
    filterFn,
    recipeComparatorFactory,
    replacements
  )
...
export const getRecipeListRecipes = createSelector(
  [
    ...
    getSelectedRecipeVariants,
    getCollectionIdForFiltering,
    getFilterFn,
    getRecipeComparatorFactory,
  ],
