...
export const getChangeCheckedRecipeHandler = ({
  ...
  collectionId,
  closeOnSelection,
}: GetChangeCheckedRecipeHandlerArgs) => (checkedRecipeId: string, isOutOfStock: boolean ): void => {
  const view = isOnDetailScreen ? 'details' : 'grid'

  dispatch(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    selectRecipeVariant(originalId, checkedRecipeId, collectionId, isOutOfStock, view, closeOnSelection) as any
  )
