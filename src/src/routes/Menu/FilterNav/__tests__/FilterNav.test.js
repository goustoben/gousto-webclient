...
describe('Filter Nav', () => {
	...
	test('will render a FilterNav', () => {
		const tree = renderer
		.create(<FilterNav menuFilterExperiment onClick={jest.fn()} ctaText="All recipes"/>)
		.toJSON()
		expect(tree).toMatchSnapshot()
	})

	test('should not render FilterNav if experiment is turned off', () => {
		wrapper = shallow(<FilterNav menuFilterExperiment={false} onClick={jest.fn()} ctaText={''} />)
		expect(wrapper.children().length).toBe(0)
	})

	test('should show filter heart loading', () => {
		wrapper = shallow(<FilterNav menuFilterExperiment onClick={jest.fn()} isLoadingHeart ifRecommendationIsSelected ctaText="All recipes" />)
	...
	test('should show filter heart', () => {
		wrapper = shallow(<FilterNav menuFilterExperiment onClick={jest.fn()} ifRecommendationIsSelected ctaText="All recipes" />)
	...
	test('should show filter heart', () => {
		wrapper = shallow(<FilterNav menuFilterExperiment onClick={jest.fn()} ctaText="All recipes" />)
	...
	test('should trigger onClick function', () => {
		...
		wrapper = shallow(<FilterNav menuFilterExperiment onClick={jest.fn()} ctaText="All recipes" onClick={mockFunction} />)
