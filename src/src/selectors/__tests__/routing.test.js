...
describe('the routing selectors', () => {
  ...
  describe('the locationAtMyGousto selector', () => {
    describe('when on the My Gousto page', () => {
      const MY_GOUSTO_PAGE = 'my-gousto'

      beforeEach(() => {
        store = generateRoutingStore(MY_GOUSTO_PAGE)
      })

      test('returns true', () => {
        expect(locationAtMyGousto(store)).toBe(true)
      })

      describe('with an alternate routing store shape', () => {
        beforeEach(() => {
          store = generateAlternateRoutingStore(MY_GOUSTO_PAGE)
        })
    ...
    describe('when not on the My Gousto page', () => {
      const NOT_MY_GOUSTO = 'not-my-gousto'

      beforeEach(() => {
        store = generateRoutingStore(NOT_MY_GOUSTO)
      })

      test('returns false', () => {
        expect(locationAtMyGousto(store)).toBe(false)
      })
