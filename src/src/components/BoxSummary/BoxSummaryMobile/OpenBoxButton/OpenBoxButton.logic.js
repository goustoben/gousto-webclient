...
const OpenBoxButton = ({ showDetails, shouldShowTutorialStep2, showTextOnButton, recipeNumber, shortlistRecipeNumber }) => {
  const iconClass = showDetails ? css.arrowDown : css.arrowUp
  const buttonText = recipeNumber === 4 ? 'Checkout' : (recipeNumber === 0 && shortlistRecipeNumber === 0) ? 'Box Summary' : 'Review Recipes'
