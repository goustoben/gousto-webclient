...
import onScreenRecovery from 'actions/onScreenRecovery'
...
describe('Subscription action', function() {
  ...
  describe('subscriptionPauseStart', function() {
    ...
    beforeEach(function() {
      sandbox = sinon.sandbox.create()
      getState = sandbox.stub().returns({
        features: Immutable.fromJS({subscriptionPause: {experiment: false, value: false}}),
        auth: Immutable.fromJS({ accessToken: 'token' }),
        user: Immutable.fromJS({ id: '123' }),
        subscriptionPause: Immutable.fromJS({
          reasons: [
    ...
    fdescribe('when feature subscriptionPause value is true', async function() {
      // let sandbox
      // let getState
      // let dispatch
      let getRecoveryContent
      beforeEach(() => {
        getState = sandbox.stub().returns({
          features: Immutable.fromJS({subscriptionPause: {experiment: false, value: true}}),
          auth: Immutable.fromJS({ accessToken: 'token' }),
          user: Immutable.fromJS({ id: '123' }),
          subscriptionPause: Immutable.fromJS({
            reasons: [
        ...
        dispatch = sandbox.spy()
        pending = sandbox.spy(statusActions, 'pending')
        getRecoveryContent = sandbox.spy(onScreenRecovery, 'getRecoveryContent')
      })
      ...
      it('should call getRecoveryContent', async function() {
        // await subPauseActions.subscriptionPauseStart()(dispatch, getState)
        // expect(getRecoveryContent).to.be.called()
        await subPauseActions.subscriptionPauseStart()(dispatch, getState)

        expect(getRecoveryContent.callCount).to.equal(1)
        expect(getRecoveryContent.firstCall).to.be.calledWithExactly(true)
      })
    })
  })
